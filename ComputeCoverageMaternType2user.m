function [pc1,pc2] = ComputeCoverageMaternType2user(P,tau_dB,Lambda,m,R)
% =======================================================================================
...
%% 1st Part %%
 ...
 %% the T-function
 % x<r 
 T_1 = @(x) 2*pi*l_p * (  chi_1(x).*integral(@(z) z.* C_1arr(x,z),0,R-x,'arrayvalued',true,'reltol',1e-3,'abstol',1e-3)+...
 ...
 % r<x<2r
 T_2 = @(x) 2*pi*l_p * ( integral(@(z) z.*chi_2(x,z).* C_2arr(x,z),x-R,R,'arrayvalued',true,'reltol',1e-3,'abstol',1e-3)+...
        integral(@(z) z.*chi_3(x,z).* C_3arr(x,z),R,x+R,'arrayvalued',true,'reltol',1e-3,'abstol',1e-3)); 
% x>2r
 T_3 = @(x) 2*pi*l_p * ( integral(@(z) z.*chi_3(x,z).* C_3arr(x,z),x-R,x+R,'arrayvalued',true,'reltol',1e-3,'abstol',1e-3));%+...
...
%% Sum term-1 
 ...
 % \int C_1(x,z)f_s(z)dz .. 
 % x<R 
 f_11 = @(x) integral(@(z)C_1arr(x,z).*chi_1(z),0,R-x, 'arrayvalued',true,'reltol',1e-3,'abstol',1e-3) + ...
              integral(@(z)C_2arr(x,z).*chi_1(z),R-x,R, 'arrayvalued',true,'reltol',1e-3,'abstol',1e-3);
 Tf_11 = @(x) f_11(x).*T_1(x);
 % R<x<2*R         
 f_12 = @(x)  integral(@(z)C_2arr(x,z).*chi_1(z),x-R,R, 'arrayvalued',true,'reltol',1e-3,'abstol',1e-3);
 Tf_12 = @(x) f_12(x).*T_2(x);
 % \int C_1(x,z)f_s(z)chi(x,z)dz ..
 % x<R 
 f_21 = @(x) integral(@(z)C_1arr(x,z).*chi_1(x).*chi_1(z),0,R-x, 'arrayvalued',true,'reltol',1e-3,'abstol',1e-3) + ...
              integral(@(z)C_2arr(x,z).*chi_2(x,z).*chi_1(z),R-x,R, 'arrayvalued',true,'reltol',1e-3,'abstol',1e-3);
 % R<x<2*R         
 f_22 = @(x)  integral(@(z)C_2arr(x,z).*chi_2(x,z).*chi_1(z),x-R,R, 'arrayvalued',true,'reltol',1e-3,'abstol',1e-3);
 %%%%%%%%%%
 % x<R 
 SumF_1 = @(x) Tf_11(x) + ...
              f_21(x) ; 
 ...
 SumF_2 = @(x) Tf_12(x) + ...
              f_22(x) ; 
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fnc1 = @(r) SumF_1(r).*M_1(r).*M_PPP(r);
fnc2 = @(r) SumF_2(r).*M_2(r).*M_PPP(r);
...
%% x<P_12^-1*R
 M_01 = @(x)  integral(@(z) ...
                          C_1arr(x,z).*chi_1(z),0,(R-P_12*x),'arrayvalued',true,'reltol',1e-3,'abstol',1e-3)...
                       +integral(@(z) ...
                          C_2arr(x,z).*chi_1(z),(R-P_12*x),R,'arrayvalued',true,'reltol',1e-3,'abstol',1e-3);
%% P_12^-1*R<x<2*P_12^-1*R
 M_02 = @(x)  integral(@(z) ...
                          C_4arr(x,z).*chi_1(z),0,P_12*x-R,'arrayvalued',true,'reltol',1e-3,'abstol',1e-3)...
            + integral(@(z) ...
                          C_2arr(x,z).*chi_1(z),P_12*x-R,R,'arrayvalued',true,'reltol',1e-3,'abstol',1e-3);
%%
 M_PPP = @(r)2*pi*l_p_2.*r.*exp(-pi*l_p_2*P_22^2.*r.^2.*(1+2*tau./(alpha-2).*hypergeom([1,1-2/alpha],2-2/alpha,-tau)));
 fnc1 = @(r) M_1(r).*M_PPP(r).*M_01(r);
 fnc2 = @(r) M_2(r).*M_PPP(r).*M_02(r);
